...
const findShortestDistance = (input) => {
  // Clean path data
  const paths = input.map(path => path.map(item => ({ direction: item[0], length: Number(item.slice(1)) })));


  const path1Positions = getPathPositions(paths[0], true);
  const path2Positions = [...getPathPositions(paths[1], true)];

  return path2Positions
    .filter((position) => path1Positions.has(position))
    .map((position) => position.split(',').map(Number))
    // formula for min manhattan distance
    .map(([x, y]) => Math.abs(x) + Math.abs(y))
    .sort((a, b) => a - b)[0];
};

findShortestDistance(paths);

// Part 2
const findSum = (input) => {
  const paths = input.map(path => path.map(item => ({ direction: item.slice(0, 1), distance: Number(item.slice(1)) })));

  const wire1Positions = getPathPositions(paths[0]);
  const wire2Positions = [...getPathPositions(paths[1]).entries()];
